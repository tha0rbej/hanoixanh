...
export function AdminDashboard({ onLogin }: { onLogin?: () => void }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [profiles, registrations, reports, campaigns, posts, messages, partners] = await Promise.all([
        ...
        tableSelect<Row>("campaign_registrations", "*, campaigns(title)", session.access_token, "&order=created_at.desc"),
  ...
  return <main className="admin-shell"><aside className="admin-sidebar"><div className="admin-brand"><span>🌱</span><div><strong>HÀ NỘI XANH</strong><small>ADMIN CONSOLE</small></div></div><nav>{([["overview", "Tổng quan"], ["people", "Tài khoản"], ["reports", "Báo cáo ô nhiễm"], ["campaigns", "Chiến dịch"], ["posts", "Bài viết"], ["points", "Điểm dọn dẹp"]] as [Tab, string][]).map(([key, label]) => <button key={key} className={tab === key ? "active" : ""} onClick={() => setTab(key)}>{label}</button>)}</nav><button className="admin-signout" onClick={() => void signOut()}>Đăng xuất</button></aside><section className="admin-content"><header className="admin-topbar"><div><p className="admin-kicker">BẢNG ĐIỀU KHIỂN</p><h1>{tab === "overview" ? "Tổng quan hoạt động" : tab === "people" ? "Tài khoản người dùng" : tab === "reports" ? "Báo cáo ô nhiễm" : tab === "campaigns" ? "Quản lý chiến dịch" : tab === "posts" ? "Quản lý bài viết" : "Quản lý điểm dọn dẹp"}</h1></div><div className="admin-user"><span>{profile.full_name || profile.email}</span><small>ADMIN</small></div></header>{error && <div className="admin-error">{error}</div>}{loading ? <div className="admin-loading">Đang tải dữ liệu…</div> : <>{tab === "overview" && <Overview rows={rows} stats={stats} />}{tab === "people" && <People profiles={rows.profiles} registrations={rows.registrations} />}{tab === "reports" && <Reports rows={rows.reports} onStatus={updateReport} />}{tab === "campaigns" && <Campaigns rows={rows.campaigns} form={campaign} setForm={setCampaign} onSubmit={createCampaign} onDelete={(id) => remove("campaigns", id)} />}{tab === "posts" && <Posts rows={rows.posts} form={post} setForm={setPost} onSubmit={createPost} onDelete={(id) => remove("posts", id)} onImage={(e, setter) => void attachImage(e, setter)} />}{tab === "points" && <Points rows={rows.points} form={point} setForm={setPoint} onSubmit={createPoint} onDelete={(id) => remove("cleanup_points", id)} />}</>}</section></main>
}

function People({ profiles, registrations }: { profiles: Row[]; registrations: Row[] }) {
  const [selectedId, setSelectedId] = useState<string | null>(null)
  const selected = profiles.find((row) => row.id === selectedId)
  const userRegistrations = selected ? registrations.filter((row) => value(row, "user_id") === selected.id && value(row, "status") !== "cancelled") : []
  return <div className="admin-page"><section className="admin-panel"><h2>Tất cả tài khoản ({profiles.length})</h2><div className="admin-table-wrap"><table><thead><tr><th>Họ tên</th><th>Email</th><th>Điện thoại</th><th>Ngày tạo</th><th>Chiến dịch</th><th></th></tr></thead><tbody>{profiles.map((row) => { const count = registrations.filter((item) => value(item, "user_id") === row.id && value(item, "status") !== "cancelled").length; return <tr key={row.id}><td>{value(row, "full_name") || "Chưa cập nhật"}</td><td>{value(row, "email")}</td><td>{value(row, "phone") || "—"}</td><td>{formatDate(row.created_at)}</td><td>{count}</td><td><button className="admin-link" onClick={() => setSelectedId(row.id)}>Xem chi tiết</button></td></tr> })}</tbody></table>{!profiles.length && <p className="admin-empty">Chưa có tài khoản.</p>}</div></section>{selected && <section className="admin-panel"><div className="admin-panel-heading"><h2>Chi tiết tài khoản</h2><button className="admin-link" onClick={() => setSelectedId(null)}>Đóng</button></div><div className="admin-detail-grid"><p><strong>Họ tên:</strong> {value(selected, "full_name") || "Chưa cập nhật"}</p><p><strong>Email:</strong> {value(selected, "email")}</p><p><strong>Điện thoại:</strong> {value(selected, "phone") || "Chưa cập nhật"}</p><p><strong>Ngày sinh:</strong> {value(selected, "birth_date") || "Chưa cập nhật"}</p><p><strong>Giới tính:</strong> {value(selected, "gender") || "Chưa cập nhật"}</p><p><strong>Địa chỉ:</strong> {value(selected, "address") || "Chưa cập nhật"}</p><p><strong>Trường học/Nghề nghiệp:</strong> {value(selected, "occupation") || "Chưa cập nhật"}</p><p><strong>Ngày tham gia:</strong> {formatDate(selected.created_at)}</p><p><strong>Chiến dịch đăng ký:</strong> {userRegistrations.length}</p></div><h3>Lịch sử chiến dịch</h3>{userRegistrations.length ? <ul>{userRegistrations.map((row) => <li key={row.id}>{value((row.campaigns as Row) || {}, "title") || value(row, "campaign_key") || "Chiến dịch"} — {value(row, "status")}</li>)}</ul> : <p className="admin-empty">Chưa đăng ký chiến dịch nào.</p>}</section>}</div>
}
